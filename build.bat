@echo off
REM ============================================================
REM  Nexus - production build script
REM
...
REM     3. Stop running Nexus / proxy processes
...
REM
REM  Flags:
REM     --clean         wipe %APPDATA%\nexus before building
...
echo.
echo === [3/9] Stopping any running Nexus / proxy processes ===
taskkill /F /IM Nexus.exe         /T >nul 2>nul
...
if exist "%APPDATA%\nexus" rmdir /s /q "%APPDATA%\nexus" >nul 2>nul
if exist "%APPDATA%\Nexus" rmdir /s /q "%APPDATA%\Nexus" >nul 2>nul
REM Leftovers from Flowseal's tray-GUI build of TgWsProxy. We now ship
REM a headless CLI mirror, but
...
REM stale secret/host (different from Nexus's), which surfaces as
...
schtasks /delete /tn "Nexus Auto Start" /f >nul 2>nul
schtasks /delete /tn "NexusAutoStart"   /f >nul 2>nul
...
set "ARTIFACTS_OK=1"
if not exist "dist\win-unpacked\Nexus.exe"   ( echo MISSING: dist\win-unpacked\Nexus.exe   & set "ARTIFACTS_OK=0" )
if not exist "dist\Nexus_x64.exe"            ( echo MISSING: dist\Nexus_x64.exe            & set "ARTIFACTS_OK=0" )
if not exist "dist\Nexus_x64-portable.7z"    ( echo MISSING: dist\Nexus_x64-portable.7z    & set "ARTIFACTS_OK=0" )
...
echo        producing a Nexus.exe with stale or wrong icons.
echo.
pause
exit /b 1
