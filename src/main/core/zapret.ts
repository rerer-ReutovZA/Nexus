 ...
 *     Nexus to a different folder, or the resources/ layout changed).
 *     Detected by comparing `BINARY_PATH_NAME` to the current .sys location.
 *     Fix: `sc delete` so winws.exe re-registers on demand.
 *
 *  2. Service registered but stopped (previous Nexus quit ran `sc stop`).
...
function ensureZapretFilters(z: ZapretConfig): void {
  const dir = zapretBundleDir()
  const utilsDir = path.join(dir, 'utils')
  if (!existsSync(utilsDir)) mkdirSync(utilsDir, { recursive: true })
  const gameFlagFile = path.join(utilsDir, 'game_filter.enabled')

  // 1. Game Filter
  if (!z.gameFilter || z.gameFilter === 'disabled') {
    if (existsSync(gameFlagFile)) rmSync(gameFlagFile, { force: true })
  } else {
    writeFileSync(gameFlagFile, z.gameFilter, 'utf-8')
  }

  // 2. IPSet Filter
  const listFile = path.join(dir, 'lists', 'ipset-all.txt')
  const backupFile = `${listFile}.backup`
  if (!existsSync(path.dirname(listFile))) mkdirSync(path.dirname(listFile), { recursive: true })

  if (z.ipsetMode === 'none') {
    if (existsSync(listFile) && !existsSync(backupFile)) {
      try {
        const content = readFileSync(listFile, 'utf-8')
        if (content.trim() !== '203.0.113.113/32' && content.trim() !== '') {
          writeFileSync(backupFile, content, 'utf-8')
        }
      } catch { /* noop */ }
    }
    writeFileSync(listFile, '203.0.113.113/32\n', 'utf-8')
  } else if (z.ipsetMode === 'any') {
    if (existsSync(listFile) && !existsSync(backupFile)) {
      try {
        const content = readFileSync(listFile, 'utf-8')
        if (content.trim() !== '203.0.113.113/32' && content.trim() !== '') {
    ...
    writeFileSync(listFile, '', 'utf-8')
  } else if (z.ipsetMode === 'loaded') {
    if (existsSync(backupFile)) {
      rmSync(listFile, { force: true })
      renameSync(backupFile, listFile)
    }
  }
}
  ...
  while (Date.now() < deadline) {
    ...
    if (winwsHealth === 'failed') {
      ...
      setStatus({
        ...
        lastError:
          winwsLastError ??
          'WinDivert не загрузился. Запустите Nexus от администратора и попробуйте снова.'
      })
      return
    }
