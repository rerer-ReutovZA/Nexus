...
const Settings: React.FC = () => {
  ...
  const [activeColorPart, setActiveColorTarget] = useState<
    'bgColor' | 'cardColor' | 'primaryColor' | 'textColor'
  >('primaryColor')
  ...
  const saveCustomTheme = () => {
    ...
    const updated = customThemes.map((t) => (t.id === editingThemeId ? { ...t, ...editTheme } : t))
  ...
  const updateGlass = async (enabled: boolean, glassEffect = appConfig?.glassEffect ?? 'mica') => {
    await patchAppConfig({ enableVibrancy: enabled, glassEffect })
    toast.message(enabled ? 'Применяем эффект стекла…' : 'Отключаем эффект стекла…', {
      description: 'Окно будет перезапущено, чтобы изменить системную подложку.'
    })
    window.setTimeout(() => appRelaunch(), 450)
  }
    ...
    <BasePage title="Настройки">
      <div className="px-4 pb-6 space-y-4">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                <Label className="text-sm">Эффект стекла</Label>
                <p className="text-xs text-muted-foreground">
                  Mica доступна на Windows 11 22H2 и новее.
                </p>
              </div>
              <Switch checked={appConfig?.enableVibrancy ?? false} onCheckedChange={updateGlass} />
            </div>
            {(appConfig?.enableVibrancy ?? false) && (
              <div className="space-y-2 rounded-lg border border-stroke/70 bg-card/45 p-3 backdrop-blur-xl">
                <Label className="text-xs text-muted-foreground">Материал окна</Label>
                <div className="grid grid-cols-3 gap-2">
                  {(
                    ...
                    <Button
                      ...
                      size="sm"
                      variant={
                        (appConfig?.glassEffect ?? 'mica') === effect ? 'default' : 'outline'
                      }
                      onClick={() => updateGlass(true, effect)}
                    >
                      {label}
                    </Button>
                  ))}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 rounded-lg border border-stroke/70 bg-card/45 p-3">
              <div className="flex items-center justify-between gap-3">
                <Label className="text-sm">Яркость фоновой карты</Label>
                <span className="text-xs tabular-nums text-muted-foreground">
                  {appConfig?.backgroundImageOpacity ?? 65}%
                </span>
              </div>
              <input
                type="range"
                min="0"
                max="100"
                value={appConfig?.backgroundImageOpacity ?? 65}
                onChange={(e) => patchAppConfig({ backgroundImageOpacity: Number(e.target.value) })}
                className="w-full accent-primary"
              />
            </div>
            <div className="space-y-2 rounded-lg border border-stroke/70 bg-card/45 p-3">
              <div className="flex items-center justify-between gap-3">
                <Label className="text-sm">Размытие фона на страницах</Label>
                <span className="text-xs tabular-nums text-muted-foreground">
                  {appConfig?.backgroundBlur ?? 48}px
                </span>
              </div>
              <input
                type="range"
                ...
                max="96"
                step="4"
                value={appConfig?.backgroundBlur ?? 48}
                onChange={(e) => patchAppConfig({ backgroundBlur: Number(e.target.value) })}
                className="w-full accent-primary"
              />
            </div>
            <div>
                      ...
                      {editingThemeId === t.id ? (
                        <div className="flex flex-col md:flex-row gap-6 p-4 border rounded-xl bg-muted/30">
                          <div className="flex-1 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {(Object.keys(colorLabels) as Array<keyof typeof colorLabels>).map(
                                (key) => (
                                  <button
                                    key={key}
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2 w-full">
                                      <div
                                        ...
                                        style={{ backgroundColor: editTheme[key] }}
                                      />
                            ...
                            <div className="flex gap-2 pt-2">
                              <Button className="flex-1 gap-1.5" onClick={saveCustomTheme}>
                        ...
                        <div className="flex items-center gap-2 group">
                          <Button
                            ...
                            style={
                              appConfig?.appTheme === t.id
                                ? {}
                                : { borderColor: t.primaryColor, backgroundColor: `${t.bgColor}33` }
                            }
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  Если выключена — при нажатии на «X» приложение полностью закрывается и завершает
                  работу.
                </span>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  Не работает — если иконка в трее выключена, при нажатии на «X» приложение
                  полностью закрывается и завершает работу.
                </span>
              </div>
              <Switch
