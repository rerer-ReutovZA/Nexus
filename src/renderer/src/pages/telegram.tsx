...
const TelegramPage: React.FC = () => {
  ...
  const handleCheckUpdate = async (): Promise<void> => {
    ...
    setChecking(true)
    try {
      const info = await tgwsCheckUpdate(true)
      setUpdateInfo(info)
      if (info.hasUpdate) {
        toast.success(`Найдено обновление TgWsProxy: v${info.latest}`)
      } else {
        toast.info('У вас установлена последняя версия TgWsProxy')
      }
    } catch (e) {
  ...
  useEffect(() => {
    tgwsCheckUpdate(false)
      .then(setUpdateInfo)
      .catch(() => setUpdateInfo(null))
  }, [])

  const showBanner =
    !!updateInfo && updateInfo.hasUpdate && !updateInfo.dismissed && !!updateInfo.assetUrl
  ...
  const installUpdate = async (): Promise<void> => {
    ...
    try {
      ...
      setUpdateInfo(fresh)
      if (res.restartError) {
        toast.warning('TgWsProxy обновлён, но не запустился', { description: res.restartError })
      } else if (res.restarted) {
        toast.success('Прокси запущен с новой версией', { style: POWER_ON_TOAST_STYLE })
      }
    } catch (e) {
  ...
  return (
    ...
    >
      <div className="px-4 pb-6 space-y-4">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-4">
            ...
            <div className="md:col-span-2 space-y-1.5">
              ...
              <Input
                value={tgws?.secret ?? ''}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, secret: e.target.value.trim() } })
                }
        ...
        <Card>
          <CardHeader>
            <CardTitle>Расширенные параметры</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="md:col-span-2 space-y-1.5">
              <Label>DC IP (по одному на строку)</Label>
              <textarea
                value={(tgws?.dcIp ?? []).join('\n')}
                onChange={(e) =>
                  patchAppConfig({
                    tgws: {
                      ...tgws!,
                ...
                placeholder={'2:149.154.167.220\n4:149.154.167.220'}
                className="min-h-20 w-full rounded-md border bg-background px-3 py-2 font-mono text-xs"
              />
              <p className="text-xs text-muted-foreground">
                Пустое поле использует значения по умолчанию. Пробелы удаляются автоматически.
              </p>
            </div>
            <div className="space-y-1.5">
              <Label>Буфер, КБ</Label>
              <Input
                type="number"
                min="1"
                value={tgws?.bufKb ?? 256}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, bufKb: Number(e.target.value) || 256 } })
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                min="1"
                value={tgws?.poolSize ?? 4}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, poolSize: Number(e.target.value) || 4 } })
                }
              />
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={tgws?.verbose ?? false}
                onChange={(e) => patchAppConfig({ tgws: { ...tgws!, verbose: e.target.checked } })}
              />{' '}
              Подробные логи
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={tgws?.cfproxy !== false}
                onChange={(e) => patchAppConfig({ tgws: { ...tgws!, cfproxy: e.target.checked } })}
              />{' '}
              Cloudflare Proxy
            </label>
            <div className="space-y-1.5">
              <Label>Свой CF-домен</Label>
              <Input
                value={tgws?.cfproxyUserDomain ?? ''}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, cfproxyUserDomain: e.target.value } })
                }
                placeholder="example.com"
              />
            </div>
            <div className="space-y-1.5">
              <Label>CF Worker-домен</Label>
              <Input
                value={tgws?.cfproxyWorkerDomain ?? ''}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, cfproxyWorkerDomain: e.target.value } })
                }
            ...
            <div className="space-y-1.5">
              ...
              <Input
                value={tgws?.fakeTlsDomain ?? ''}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, fakeTlsDomain: e.target.value.trim() } })
                }
                placeholder="example.com"
              />
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={tgws?.proxyProtocol ?? false}
                onChange={(e) =>
                  patchAppConfig({ tgws: { ...tgws!, proxyProtocol: e.target.checked } })
                }
              />{' '}
        ...
        {status.lastError && (
          <Card className="border-red-500/50">
            <CardContent className="pt-4">
              ...
              {status.lastError.includes('certifi') && (
                <Button
                  variant="outline"
                  size="sm"
                  className="mt-3"
                  onClick={() => {
                    void tgwsRestoreBundled()
                      .then((res) => {
                        if (res.restartError) throw new Error(res.restartError)
                        toast.success('Встроенная версия TgWsProxy восстановлена', {
                          style: POWER_ON_TOAST_STYLE
                        })
                      })
                      .catch((e) =>
