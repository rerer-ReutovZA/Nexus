...
Nexus is distributed under the terms of the GNU General Public License
...
If you redistribute Nexus (in source or binary form) you must preserve
...
Tailwind theme tokens and various visual primitives in Nexus's renderer
...
GPL applies to those files exactly as it does to the rest of Nexus.

> Nexus's choice of GPL-3.0 as its overall license is a direct
...
Bundled `TgWsProxy_windows.exe` is a headless CLI rebuild of the official
source. The official Flowseal release ships a tray-GUI binary (entry point
`windows.py` + `pystray`) that pops a first-run dialog, parks a tray
icon, ignores `--host`/`--port`/`--secret` CLI args and acquires a
single-instance mutex incompatible with Nexus's headless integration.
Nexus rebuilds the exact same source through PyInstaller against
`proxy.tg_ws_proxy:main` (the CLI entry point), producing a true headless
`.exe` with no UI surface. No source code is modified — only the build
configuration.
...
Nexus redistributes the bundle verbatim, plus a small set of
Nexus-specific patches applied at build time by
`scripts/build-zapret/build.bat`. The patches are documented in that
script and add `--profile` plumbing needed by Nexus's strategy
...
Nexus distributes WinDivert as an unmodified binary alongside
...
Nexus started life as a fork of a Russian community fork of
**Sparkle**, and has since been rewritten end-to-end. None of the
original Sparkle source survives in current Nexus code, but the
...
form by Nexus, and are governed by their own licenses (predominantly
