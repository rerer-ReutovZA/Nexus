!macro customInstall
  ...
  ; Publisher column — kept as "Nexus" so the table column reads cleanly
  ...
  WriteRegStr SHCTX "${UNINSTALL_REGISTRY_KEY}" "Publisher" "Nexus"
  WriteRegStr SHCTX "${UNINSTALL_REGISTRY_KEY}" "Comments" "lazzy & cherry"

  ; Ensure the icon shown next to Nexus in P&F is *our* fresh icon,
  ; rebuilt from design/icon.svg into resources/icon.ico on every build.
  ; Pointing at the standalone .ico (instead of "Nexus.exe,0") sidesteps
  ...
  ; pair makes Programs & Features leave the column blank for Nexus.
...
!macro customUnInstall
  ...
  ; want to delete every trace Nexus ever left behind on the machine, so
  ...
  ; launches a newer installer, it first writes `$INSTDIR\.nexus-upgrade`.
  ...
  IfFileExists "$INSTDIR\.nexus-upgrade" nexus_skip_wipe 0

  ; 1. Stop any running Nexus / Zapret / proxy processes that could keep
  ...
  ; 2. Remove any auto-start Scheduled Tasks Nexus may have created via
  ;    the in-app "Run on login" toggle. Both legacy and current task names
  ;    are covered.
  nsExec::ExecToLog 'schtasks /delete /tn "Nexus Auto Start" /f'
  nsExec::ExecToLog 'schtasks /delete /tn "NexusAutoStart" /f'

  ; 3. Remove run-on-login Registry keys (alternative auto-start mechanism).
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Nexus"
  DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "Nexus"

  ; 4. Remove the URI-scheme protocol handlers Nexus registers at runtime
  ;    (nexus://, tg-ws:// — see src/main/utils/init.ts).
  ...
  ; Nexus's userData lives in `nexus` (production) and `nexus-dev`
  ...
  ; NOT touch any other folder names — Nexus is designed to coexist
  ...
  ;    C:\Users\* and remove any Nexus userData under each profile.
  ...
  user_loop:
    ...
    RMDir /r "$0\$2\AppData\Roaming\nexus"
    RMDir /r "$0\$2\AppData\Local\nexus"
    RMDir /r "$0\$2\AppData\Roaming\nexus-dev"
    RMDir /r "$0\$2\AppData\Local\nexus-dev"
  user_next:
    FindNext $1 $2
